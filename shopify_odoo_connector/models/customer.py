...
class Partners(models.Model):
    ...
    shopify_ins = fields.Many2one('shopify.configuration',
                                  string="Shopify Instance")
    synced_customer = fields.Boolean(readonly=True, store=True)

    def sync_shopify_customer(self):
        api_key = self.shopify_ins.con_endpoint
        PASSWORD = self.shopify_ins.consumer_key
        store_name = self.shopify_ins.shop_name
        version = self.shopify_ins.version
